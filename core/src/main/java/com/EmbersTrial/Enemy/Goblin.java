...
public class Goblin {
    ...
    public void takeDamage(int damage) {
        health -= damage;
        public void takeDamage(int damage) {
            if (!isAlive || timeSinceLastDamage < damageCooldown) return;

            health -= damage;
            timeSinceLastDamage = 0f; // Reset cooldown
            isFlashing = true; // Trigger flash effect
            flashTimer = 0f;
            renderer.setColor(Color.RED); // Change color to indicate damage
